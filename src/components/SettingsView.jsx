...
export default function SettingsView({ settings, updateSettings, ytdlpOk, setYtdlpOk }) {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      {/* App options */}
      <div className="card mb-4">
        {/* YouTube Cookies for 4K */}
        <div className="rounded-2xl p-4 mb-4" style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.08)'}}>
          <p className="text-sm font-semibold text-white mb-1">🍪 YouTube cookies (4K)</p>
          <p className="text-xs mb-3" style={{color:'rgba(255,255,255,0.4)'}}>
            Uloguj se u YouTube u Browser tabu → klikni Izvezi → program može skinuti 4K
          </p>
          <div className="flex items-center gap-3 flex-wrap">
            <label className="flex items-center gap-2 cursor-pointer">
              <div onClick={()=>updateSettings({useBrowserCookies:!settings?.useBrowserCookies})}
                className="relative w-10 h-6 rounded-full transition-all"
                style={{background:settings?.useBrowserCookies?'#7c3aed':'rgba(255,255,255,0.15)'}}>
                <div className="absolute top-1 w-4 h-4 rounded-full bg-white transition-all"
                  style={{left:settings?.useBrowserCookies?'22px':'4px'}}/>
              </div>
              <span className="text-sm" style={{color:'rgba(255,255,255,0.7)'}}>Koristi cookies za skidanje</span>
            </label>
            <button onClick={async()=>{
                const res = await api?.exportCookies?.()
                if(res?.success) setCookieMsg('✓ '+res.count+' cookies — 4K aktiviran!')
                else setCookieMsg('⚠ '+(res?.error||'Nisi ulogovan u Browser tabu'))
                setTimeout(()=>setCookieMsg(''),4000)
              }}
              className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold hover:opacity-80"
              style={{background:'rgba(124,58,237,0.2)',color:'#c4b5fd',border:'1px solid rgba(124,58,237,0.4)'}}>
              🍪 Izvezi YouTube cookies
            </button>
          </div>
          {cookieMsg && (
            <p className="text-xs mt-2" style={{color:cookieMsg.startsWith('✓')?'#4ade80':'#f87171'}}>
              {cookieMsg}
            </p>
          )}
        </div>
